...
/***/ (function(module, exports, __webpack_require__) {
	...
	var border = Math.max(window.innerHeight, window.innerWidth);
	// http://stackoverflow.com/a/7557433/2422398
	function isElementInViewport(el) {
	    var rect = el.getBoundingClientRect();
	    return (rect.bottom >= -border &&
	        rect.right >= -border &&
	...
	function replaceScriptWithWidget(el, callback) {
	    var iframe = document.createElement('iframe');
	    var height = getDataHeight(el);
	    var previewWidth = el.getAttribute('data-preview-width');
	    ...
	    function makeWidget(sketch) {
	        ...
	        iframe.addEventListener("load", function () { return callback && callback(iframe); });
	        if (el.parentNode)
	...
	function lazilyReplaceScriptWithWidget(el, callback) {
	    var height = getDataHeight(el);
	    ...
	    whenVisible(el, function (el) { return replaceScriptWithWidget(el, callback); });
	}
	function lazilyReplaceAllScriptsWithWidget(callback) {
	...
	if (autoload) {
	    if (document.readyState === 'complete') {
	        ...
	        window.addEventListener('load', function () { return lazilyReplaceAllScriptsWithWidget(); }, false);
	    }
	}
	window['p5Widget'] = {
	    baseURL: myBaseURL,
